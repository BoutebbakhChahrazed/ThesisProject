...
async function fetchWeatherAlerts(lat: number, lon: number): Promise<WeatherAlert[]> {
  ...
  if (wind > WIND_DANGER_MS) {
    alerts.push({ level: "danger", icon: "🌬️", message: `Wind speed ${wind.toFixed(1)} m/s — drone flight is unsafe` });
  } else if (wind > WIND_WARNING_MS) {
    alerts.push({ level: "warning", icon: "🌬️", message: `Wind speed ${wind.toFixed(1)} m/s — fly with caution` });
  }

  // Rain
  if (rain > RAIN_THRESHOLD) {
    alerts.push({ level: "danger", icon: "🌧️", message: `Rain detected (${rain.toFixed(1)} mm/h) — flight not recommended` });
  }

  // Clouds
  if (cloud > CLOUD_WARNING) {
    alerts.push({ level: "warning", icon: "☁️", message: `Cloud cover ${cloud}% — multispectral image quality may be poor` });
  }

  // Heat
  if (temp > HEAT_DANGER_C) {
    alerts.push({ level: "danger", icon: "🌡️", message: `Temperature ${temp.toFixed(1)}°C — extreme heat risk for potato crop` });
  } else if (temp > HEAT_WARNING_C) {
    alerts.push({ level: "warning", icon: "🌡️", message: `Temperature ${temp.toFixed(1)}°C — monitor crop for heat stress` });
  }
...
function WeatherAlertBanner({ fieldId, fields }: { fieldId: string; fields: Field[] }) {
  ...
  if (status.loading) {
    return (
      <div className="rounded-xl border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground animate-pulse">
  ...
  if (status.error) {
    return (
      <div className="rounded-xl border border-amber-300 bg-amber-50 text-amber-700 px-4 py-3 text-sm">
        ⚠️ {status.error}
      </div>
    );
  }
  ...
  if (status.alerts.length === 0) {
    return (
      <div className="rounded-xl border border-green-300 bg-green-50 text-green-700 px-4 py-3 text-sm">
...
export default function Flights() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const matchedDrone = dronesData.find((d) => d.field_id === initialField);
  ...
  const onFieldChange = async (field_id: string) => {
    const matchedDrone = drones.find((d) => d.field_id === field_id);
  ...
  const onCreate = async () => {
    ...
    if (!draft.drone_id) { setError("No drone is assigned to this field — assign one in the Drones page first"); return; }
  ...
  const assignedDrone = useMemo(() => drones.find((d) => d.field_id === draft.field_id), [drones, draft.field_id]);

  return (
    <div className="space-y-4">
      <PageHeader title="🛰️ Flights" subtitle="Create and manage drone flights per field" gradient="gradient-gallery" />
      ...
      <Card className="p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          ...
          <div className="space-y-2">
            ...
            <Input
              value={assignedDrone?.drone_name || (draft.field_id ? "No drone assigned" : "—")}
              readOnly
      ...
      {creating && (
        <Card className="p-5 space-y-4">
          ...
          {!assignedDrone && draft.field_id && (
            <div className="rounded-xl border border-red-300 bg-red-50 text-red-700 px-4 py-3 text-sm">
              ⚠️ No drone assigned to this field. Go to the{" "}
              <a href="/drones" className="underline font-medium">Drones page</a> and assign one first.
            </div>
          )}
