...
import { AlertTriangle, Bell, Thermometer, Wind } from "lucide-react";
  ...
  return (
    <div className="bg-card rounded-2xl shadow-soft border border-border/40 overflow-hidden">
      <div className="px-4 py-3 border-b border-border/50 flex items-center justify-between">
        <h3 className="font-display font-semibold">🔔 Live Detections</h3>
      ...
      <div className="max-h-[320px] overflow-y-auto divide-y divide-border/40">
        {items.length === 0 && (
          <div className="p-6 text-center text-sm text-muted-foreground">All zones healthy! Your crops are thriving 🌿</div>
        )}
        {items.map((it) => {
