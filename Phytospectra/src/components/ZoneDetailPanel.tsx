...
import { ZoneData, healthLabel, stressLabel, stressEmoji } from "@/lib/mockData";
    ...
    <>
      ...
      <div className="absolute right-0 top-0 h-full w-full md:w-[400px] bg-card z-[1001] shadow-card animate-slide-in-right overflow-y-auto">
        ...
        <div className="px-5 pt-4 flex gap-2">
          {(["photo", "heat"] as const).map(k => (
            <button key={k} onClick={() => setTab(k)}
              className={`flex-1 px-3 py-2 rounded-xl text-xs font-semibold transition-smooth ${tab === k ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
              {k === "photo" ? "📷 Drone Photo" : "🌡️ Heat Analysis"}
            </button>
          ))}
        </div>
        <div className="p-5">
          ...
          <div className="mt-5 text-center">
            ...
            <div className="mt-2 text-sm font-semibold flex items-center justify-center gap-2">
              <span>{stressEmoji(zone.stress_class)}</span>{stressLabel(zone.stress_class)}
            </div>
          </div>
          <div className="mt-4">
