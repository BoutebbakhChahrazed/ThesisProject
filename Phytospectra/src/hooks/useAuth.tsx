...
interface AuthCtx {
  ...
  role: AppRole | null;
  profile: { display_name: string | null; farm_name: string | null; specialty: string | null } | null;
  loading: boolean;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchMeta = async (uid: string, currentSession: Session | null) => {
    // Skip if already fetching or already loaded for this user
    if (fetchingRef.current || fetchedUidRef.current === uid) return;
    fetchingRef.current = true;

    try {
      const [{ data: r }, { data: p }] = await Promise.all([
        supabase.from("user_roles").select("role").eq("user_id", uid).maybeSingle(),
        supabase.from("profiles").select("display_name, farm_name, specialty").eq("user_id", uid).maybeSingle(),
      ]);

      const resolvedRole = (r?.role as AppRole | null) ?? roleFromMetadata(currentSession);
      setRole(resolvedRole);
      setProfile(p ?? null);
      fetchedUidRef.current = uid; // Mark as done for this UID
    } catch (err) {
  ...
  return (
    <Ctx.Provider value={{ user, session, role, profile, loading, signOut }}>
      {children}
    </Ctx.Provider>
  );
}
